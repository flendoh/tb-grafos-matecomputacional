...
class PreviewGraph:
    ...
    def save_img(self, start_node=0, end_node=0):
        ...
        if start_node!=end_node:
            ...
            nx.draw_networkx_nodes(self.graph, pos, nodelist=shortest_path, node_color='orange', node_size=700)
    ...
    def update(self, matrix, start_node, end_node):
        ...
        dpg.set_value(self.text_info, str(self.graph.edges))
    ...
    def display_graph(self):
        with dpg.window(label="Grafo asociado a la matriz", width=700, height=600) as self.parent:
            dpg.add_text("Elementos del grafo:")
            self.text_info = dpg.add_text(str(self.graph.edges))
